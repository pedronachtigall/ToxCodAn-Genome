...
def _UniprotSearch_(outF, uniprot, cpu, keeptemp):
    #blast search
    sp.check_output("makeblastdb -dbtype prot -in "+uniprot+" -out "+outF+"blastDB/uniprot", shell=True)
    sp.call("blastx -num_threads "+str(cpu)+" -evalue 0.01 -query "+outF+"toxin_annotation_cds.fasta -db "+outF+"blastDB/uniprot -out "+outF+"uniprot_blast.out -outfmt \'6 qseqid sseqid pident length evalue qstart qend qlen sstart send slen salltitles\'", shell=True)

    #getting best hit
    orftox = {}
    blast = pd.read_csv(outF+"uniprot_blast.out",sep='\t',names=['qseqid','sseqid','pident','length','evalue','qstart','qend','qlen','sstart','send','slen','salltitles'])

    #filter identity >=50%
    blast2 = blast[blast['pident'] >= 50]

    #filter based on q coverage >= 50%
    for index, row in blast2.iterrows():
        orftox.setdefault(row['qseqid'], [0.0, 0.0, 0.0, 0.0, "No Hit"])
        #print(row['qseqid'], row['sseqid'],row['pident'],row['salltitles'])
        if row['length'] >= (row['qlen']/3)*0.5:
            if row['pident'] >= orftox[row['qseqid']][0]:
    ...
    if keeptemp == "False" or keeptemp == False:
        os.system("rm -r "+outF+"blastDB/")
        os.system("rm "+outF+"uniprot_blast.out")
...
def __main__():
    ...
    parser.add_option("-u", "--uniprot", dest="uniprot", help="Optional - path to uniprot/toxprot database to perform an extra step of similarity search to compare the annotated toxins to the uniprot/toxprot database. It will output a report file in TXT format containing the annotated toxin and the best hit in the uniprot/toxprot database, alongside their percent identity, alignment length, toxin length, and uniprot/toxprot entry length.", metavar="fasta", default=None)
...
░ ▓██▓ ░ ▒██   ██░ ░ █ █ ▒ ▒▓▓▄ ▄██▒▒██   ██░░▓█▄   ▌░██▄▄▄▄██ ▓██▒  ▐▌██▒
  ...
  ░      ░ ░ ░ ▒   ░    ░  ░        ░ ░ ░ ▒   ░ ░  ░   ░   ▒      ░   ░ ░
    ...
    ***Warning***
        ...
        if options.uniprot != None:
            print("\n"+dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+" >>>> comparing annotated toxins to the uniprot/toxprot database...")
            print("\tUniprot/Toxprot database ->", options.uniprot)

            _UniprotSearch_(options.output, options.uniprot, str(options.cpu), str(options.keeptemp))

            print("\n\t>>> Check the uniprot/toxprot report: "+options.output+"uniprot_report.txt")
